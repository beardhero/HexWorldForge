...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] HexPlayerNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is HexPlayerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (HexPlayerNetworkObject.Length > 0 && HexPlayerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(HexPlayerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<HexPlayerBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateHexPlayer instead, its shorter and easier to type out ;)")]
		public HexPlayerBehavior InstantiateHexPlayerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(HexPlayerNetworkObject[index]);
			var netBehavior = go.GetComponent<HexPlayerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<HexPlayerBehavior>().networkObject = (HexPlayerNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of HexPlayer
		/// </summary>
		/// <returns>
		/// A local instance of HexPlayerBehavior
		/// </returns>
		/// <param name="index">The index of the HexPlayer prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public HexPlayerBehavior InstantiateHexPlayer(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(HexPlayerNetworkObject[index]);
			var netBehavior = go.GetComponent<HexPlayerBehavior>();
